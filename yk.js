...
var RSA=function(t, e, n) {
    ...
    h.prototype.convert = function(t) {
        ...
        if (!(o.t <= 0)) {
            ...
            if (0 != p) {
                var h = p * (1 << this.F1) + (f > 1 ? s[f - 2] >> this.F2 : 0),
                    ...
                    M = v - f,
                    w = null == e ? r() : e;
                for (s.dlShiftTo(M, w), n.compareTo(w) >= 0 && (n[n.t++] = 1, n.subTo(w, n)), i.ONE.dlShiftTo(f, w), w.subTo(s, s); s.t < f;)
                s[s.t++] = 0;
                for (; --M >= 0;) {
                    var b = n[--v] == p ? this.DM : Math.floor(n[v] * g + (n[v - 1] + m) * y);
                    if ((n[v] += s.am(0, b, n, M, 0, f)) < b) for (s.dlShiftTo(M, w), n.subTo(w, n); n[v] < --b;)
                    n.subTo(w, n)
                }
        ...
        if (a-- > 0) for (s < this.DB && (n = this[a] >> s) > 0 && (i = !0, r = l(n)); a >= 0;)
        s < e ? (n = (this[a] & (1 << s) - 1) << e - s, n |= this[--a] >> (s += this.DB - e)) : (n = this[a] >> (s -= e) & o, s <= 0 && (s += this.DB, --a)), n > 0 && (i = !0), i && (r += l(n));
        return i ? r : "0"
    ...
    if (null == v) {
        ...
        if (v = new Array, M = 0, window.crypto && window.crypto.getRandomValues) {
            ...
            for (window.crypto.getRandomValues(C), N = 0; N < 32; ++N)
            v[M++] = C[N]
        }
        ...
        for (; M < w;)
        N = Math.floor(65536 * Math.random()), v[M++] = N >>> 8, v[M++] = 255 & N;
        M = 0, b()
    }

    function I() {
        if (null == m) {
            for (b(), (m = new y).init(v), M = 0; M < v.length; ++M)
            v[M] = 0;
            M = 0
        }
        return m.next()
